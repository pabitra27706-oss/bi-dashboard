...
interface Props {
  tables: TableInfo[];
  activeTable: string;
  onSwitch: (tableName: string) => void;
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
      >
        ...
        {activeInfo && (
          <span className="text-[10px] text-slate-400">
            ({activeInfo.row_count.toLocaleString()})
          </span>
        )}
        <svg className={`w-3 h-3 text-slate-400 transition-transform ${open ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    {isActive ? (
                      <svg className="w-4 h-4 text-indigo-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
