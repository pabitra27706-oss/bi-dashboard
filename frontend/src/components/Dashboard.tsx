...
function WelcomeUploadModal({
  ...
  return (
    <div
      className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 dark:bg-black/70 backdrop-blur-lg animate-[fade-in_0.3s]"
      onClick={onSkip}
    >
      <div
        className="relative w-full max-w-2xl mx-4 max-h-[92vh] overflow-y-auto glass-card rounded-2xl shadow-2xl animate-[scale-in_0.35s] border border-[var(--color-border)]"
      ...
      >
        ...
        <button
          type="button"
          onClick={onSkip}
          className="absolute top-4 right-4 z-10 flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-[var(--color-border)] dark:border-gray-500 text-[var(--color-muted-foreground)] dark:text-gray-300 hover:bg-[var(--color-surface-3)] dark:hover:bg-gray-700 hover:text-[var(--color-foreground)] dark:hover:text-white transition-all active:scale-[0.97]"
          aria-label="Skip"
        >
        ...
        {!done && !preview && !loadingPreview && !loadingSample && (
          <div className="px-8 pb-6">
            <div className="flex items-center justify-center gap-2 pt-4 border-t border-[var(--color-border)]">
              <button type="button" onClick={onSkip} className="text-xs text-[var(--color-muted)] dark:text-gray-400 hover:text-[var(--color-foreground)] dark:hover:text-white transition-colors font-medium">
