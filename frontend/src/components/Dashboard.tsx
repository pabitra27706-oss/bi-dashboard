            ...
            <div className="space-y-5 animate-[fade-in-up_0.3s]">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {preview.columns.map((col, i) => (
                    <span
                      key={i}
                      className="px-2.5 py-1 text-[10px] rounded-md bg-[var(--color-surface-2)] border border-[var(--color-border)] text-[var(--color-foreground)]"
                    >
                      {col.name}{" "}
                      <span className="text-[var(--color-muted)]">({col.type})</span>
                    </span>
                  ))}
                </div>
              </div>
                      ...
                      <thead className="bg-[var(--color-surface-2)]">
                        <tr>
                          {preview.columns.map((col, i) => (
                            <th
                              key={i}
                              className="px-2.5 py-2 text-left font-semibold text-[var(--color-muted-foreground)] whitespace-nowrap"
                            >
                              {col.name}
                            </th>
                          ))}
                        ...
                        {preview.sample_rows.map((row, ri) => (
                          ...
                          >
                            {preview.columns.map((col, ci) => (
                              <td
                                key={ci}
                                className="px-2.5 py-1.5 text-[var(--color-foreground)] whitespace-nowrap font-mono max-w-[150px] truncate"
                              >
              ...
              <div className="flex gap-3 pt-1">
                <button
                  ...
                  onClick={resetFile}
                  disabled={uploading}
                  className="flex-1 px-4 py-3 text-sm font-medium rounded-xl border border-[var(--color-border)] text-[var(--color-muted-foreground)] hover:bg-[var(--color-surface-3)] transition-all disabled:opacity-50"
                >
                  Change file
                </button>
                <button
                  ...
                  disabled={uploading}
                  className="flex-1 px-4 py-3 text-sm font-semibold rounded-xl btn-primary disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {uploading ? (
                    <>
          ...
          <div className="px-8 pb-6">
            <div className="flex items-center justify-center gap-2 pt-4 border-t border-[var(--color-border)]">
              <button
                type="button"
                onClick={onSkip}
                className="text-xs text-[var(--color-muted)] hover:text-[var(--color-foreground)] transition-colors font-medium"
              >
...
function AssistantBlock({
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      await doExportPDF(resp, chartRef.current, activeTable, isDark);
    } catch (e) {
      console.error(e);
    } finally {
      setExporting(false);
    ...
    return (
      <div className="flex items-start gap-3 animate-[fade-in-up_0.4s]">
        ...
        <div className="flex-1 glass-card rounded-2xl rounded-tl-sm p-5">
          ...
          <button
            type="button"
            onClick={() => onDrillDown(resp.query)}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[var(--color-destructive)] hover:underline"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
      ...
      <div className="flex-1 min-w-0 space-y-3">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs font-bold text-[#22c55e]">MATRIQ</span>
          </div>
          ...
          {resp.execution_time > 0 && (
            <span className="badge badge-gray mono-sm">{resp.execution_time}s</span>
          )}
          {(hasData || resp.insight) && (
            <button
              ...
              disabled={exporting}
              className="ml-auto btn-primary text-xs py-1.5 px-3.5 flex items-center gap-1.5"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          {!isLatest && (
            <button
              ...
              className="btn-icon text-[var(--color-muted)]"
        ...
        {collapsed ? (
          <div
            className="text-xs text-[var(--color-muted-foreground)] line-clamp-2 cursor-pointer hover:text-[var(--color-foreground)] transition-colors rounded-xl bg-[var(--color-surface-2)] p-3 border border-[var(--color-border)] glass-card-interactive"
          ...
          <div className="space-y-4">
            <ErrorBoundary fallbackTitle="KPI error">
              <div style={{ animation: "fade-in-up 0.4s both" }}>
              ...
              <ErrorBoundary fallbackTitle="Chart error">
                <div className="glass-card rounded-xl overflow-hidden" style={{ animation: "fade-in-up 0.4s 0.1s both" }}>
                  ...
                  <div className="flex items-center justify-between px-5 pt-4 pb-2 flex-wrap gap-2">
                    <div>
                      <h3 className="text-sm font-semibold text-[var(--color-foreground)]">
                        {resp.chart_config!.title || "Results"}
                      </h3>
                      <p className="text-[10px] text-[var(--color-muted)] mt-0.5 mono-sm">
                        {resp.data.length} pts &middot; {curType} &middot; {resp.execution_time}s
                      </p>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        type="button"
                        onClick={() => setFullscreen({ config: resp.chart_config!, data: resp.data })}
                        className="btn-icon border border-[var(--color-border)]"
                  ...
                  <div ref={chartRef} className="px-3 pb-3 print-chart">
                    <ChartRenderer
                      data={resp.data}
                      config={resp.chart_config!}
                      darkMode={darkMode}
                      overrideChartType={chartType || undefined}
                      onDrillDown={(xVal, xField) =>
                        onDrillDown(`Show me a detailed breakdown for ${xField} = "${xVal}"`)
                      }
                    />
                  </div>
                </div>
                  ...
                  {resp.additional_charts.map((ac, i) => (
                    <div key={i} className="glass-card rounded-xl p-4 overflow-hidden">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                          type="button"
                          onClick={() => setFullscreen({ config: ac, data: resp.data })}
                          className="btn-icon"
                          aria-label="Fullscreen"
                        >
                          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            ...
            <div style={{ animation: "fade-in-up 0.4s 0.15s both" }}>
              <InsightBox
                insight={resp.insight}
                sql={resp.sql}
                executionTime={resp.execution_time}
                confidence={resp.confidence}
                assumptions={resp.assumptions}
              />
            </div>

            <ErrorBoundary fallbackTitle="Table error">
              <div style={{ animation: "fade-in-up 0.4s 0.2s both" }}>
...
function DashboardInner() {
  ...
  const conversationCtx = useMemo(
    () =>
      messages
        .filter((m) => m.role === "assistant" && m.response?.sql)
        .slice(-5)
        .map((m) => ({
          query: m.query,
          sql: m.response!.sql,
          insight: m.response!.insight,
          chart_config: m.response!.chart_config as unknown,
        })),
    [messages]
  );
  ...
  useEffect(() => {
    try {
      localStorage.setItem("bi-conversations", serializeConversations(conversations));
    } catch {
      try {
        localStorage.setItem("bi-conversations", serializeConversations(conversations.slice(0, 20)));
      } catch {
        // silently fail
      }
    }
  ...
  const handlePinConv = useCallback(
    (id: string) => {
  ...
  const handleArchiveConv = useCallback(
    (id: string) => {
      ...
      if (activeConvId === id) {
        const rem = conversations.filter((c) => c.id !== id && !c.isArchived);
        setActiveConvId(rem.length ? rem[0].id : null);
      }
    },
  ...
  const handleDeleteTable = async (name: string) => {
    try {
      const r = await apiDeleteTable(name);
      setTableList(r.tables);
      setActiveTable(r.active_table);
      if (activeTable === name) handleNewChat();
      fetchSchema().then(setSchema).catch(console.error);
    } catch (e) {
      console.error(e);
    }
        ...
        if (resp.is_modification) {
          updateConversation(capturedConvId, (c) => {
            ...
            for (let i = msgs.length - 1; i >= 0; i--) {
              if (msgs[i].role === "assistant" && msgs[i].response) {
                msgs[i] = { ...msgs[i], response: resp, isCollapsed: false };
                break;
              }
            }
    ...
    <div className="flex h-screen overflow-hidden noise-bg">
      {showWelcomeUpload && (
        <WelcomeUploadModal
          ...
          onSkip={() => setShowWelcomeUpload(false)}
        />
      )}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative z-10">
        <header className="flex items-center justify-between px-5 py-2.5 glass-header z-20 print-hidden">
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-[var(--color-border)] text-[var(--color-muted-foreground)] hover:bg-[#22c55e]/5 hover:border-[#22c55e]/25 hover:text-[#22c55e] transition-all active:scale-[0.97]"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            ...
            <div className="hidden sm:flex items-center gap-2.5 ml-1">
              ...
              <div>
                ...
                <p className="text-[9px] text-[var(--color-muted)] font-mono uppercase mt-0.5">
                  {schema ? `${schema.row_count.toLocaleString()} rows` : "Analytics"}
                </p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={toggle}
              className="p-2 rounded-lg border border-[var(--color-border)] text-[var(--color-muted-foreground)] hover:bg-[var(--color-surface-3)] hover:text-[var(--color-foreground)] transition-colors"
                ...
                <div className="relative z-10">
                  <div className="mb-8 animate-[scale-in_0.5s] mx-auto animate-[glow-pulse_4s_infinite]">
                    <img
                      src="/logo.png"
                      alt="MATRIQ"
                      className="w-20 h-20 rounded-2xl object-contain shadow-2xl shadow-[#22c55e]/20 mx-auto"
                    />
                  </div>
                  ...
                  {tableList.length === 0 && (
                    <div
                      ...
                      style={{ animation: "fade-in-up 0.5s 0.2s both" }}
                    >
                      <button
                        type="button"
                        onClick={() => setShowWelcomeUpload(true)}
                        className="btn-primary text-sm py-3 px-8 flex items-center gap-2 shadow-lg shadow-[#22c55e]/25"
                      >
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V16.5m-13.5-9L12 3m0 0 4.5 4.5M12 3v13.5" />
                        </svg>
                      ...
                      <p className="text-[11px] text-[var(--color-muted)]">
                        or use the Upload CSV button in the header
                      </p>
                    </div>
                  )}
                  ...
                  {schema && tableList.length > 0 && (
                    <div
                      className="flex items-center gap-3 mb-10 text-xs flex-wrap justify-center"
                      style={{ animation: "fade-in-up 0.5s 0.2s both" }}
                    >
                      <span className="badge badge-green py-1 px-3 text-[11px]">
                        {(activeTableInfo?.row_count ?? schema.row_count).toLocaleString()} records
                      </span>
                      <span className="badge badge-blue py-1 px-3 text-[11px]">
                  ...
                  <div className="grid sm:grid-cols-3 gap-4 max-w-3xl w-full">
                    {suggestions.slice(0, 3).map((s, i) => (
                      <button
                        ...
                        className="text-left p-5 rounded-xl glass-card glass-card-hover group"
                        style={{ animation: `fade-in-up 0.5s ${0.3 + i * 0.08}s both` }}
                      >
                        <span
                        ...
                        <p className="text-sm text-[var(--color-muted-foreground)] leading-relaxed group-hover:text-[var(--color-foreground)] transition-colors">
                          {s.query}
                        </p>
                  ...
                  {suggestions.length > 3 && (
                    <div
                      className="mt-5 flex flex-wrap gap-2 max-w-2xl justify-center"
                      style={{ animation: "fade-in-up 0.5s 0.5s both" }}
                    >
                      {suggestions.slice(3).map((s, i) => (
                        <button
                          ...
                          type="button"
                          onClick={() => handleSubmit(s.query)}
                          className="px-3.5 py-2 text-xs rounded-lg border border-[var(--color-border)] text-[var(--color-muted-foreground)] hover:border-[#22c55e]/20 hover:text-[#22c55e] transition-all"
                        >
                          {s.query}
                        </button>
                      ))}
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        {uploadSuggestions.map((q, i) => (
                          <button
                            ...
                            className="px-3.5 py-2 text-xs rounded-lg border border-[#22c55e]/15 text-[#22c55e] hover:bg-[#22c55e]/5 transition-colors"
                          >
                            {q}
                          </button>
                        ))}
                ...
                return (
                  <div key={msg.id} className="flex justify-end animate-[slide-in-right_0.3s]">
                    <div className="max-w-[75%] flex items-start gap-3">
                      <div>
                        <div className="bg-[#22c55e] text-white rounded-2xl rounded-tr-sm px-5 py-3 shadow-lg shadow-[#22c55e]/10">
                          <p className="text-sm leading-relaxed">{msg.query}</p>
                        </div>
                        <p className="text-[9px] text-[var(--color-muted)] text-right mt-1 mr-1 font-mono tabular-nums">
        ...
        <div className="glass-header px-6 py-3 print-hidden">
          <ChatInput
            ...
            followUps={[...followUps, ...uploadSuggestions]}
            activeTable={activeTable}
