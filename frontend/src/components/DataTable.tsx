...
export default function DataTable({ data, visible, onToggle }: Props) {
  ...
  return (
    <div>
      <button
        onClick={onToggle}
        className="flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors mb-3"
      >
                ...
                <tr>
                  {columns.map((col) => (
                    <th
                      ...
                      className="px-3 py-2 text-left font-medium text-slate-500 dark:text-slate-400 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700 select-none whitespace-nowrap"
                    >
                      {col}{" "}
                      {sortKey === col
                        ? sortAsc
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="px-2 py-1 text-[10px] rounded bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-30"
                >
                  Prev
                </button>
                <button
